...
def _resolve_workspace_config_file(
    ...
    Examples
    --------
    Resolve ``./tmux.conf`` from the workspace file directory:
...
def _load_here_in_current_session(builder: WorkspaceBuilderProtocol) -> None:
    ...
    Examples
    --------
    Resolving the attached session and building into it depends on live
    ``$TMUX`` state, so this is exercised in the CLI integration tests:
