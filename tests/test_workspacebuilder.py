...
def test_split_windows_three_pane(session):
    yaml_config = read_config_file("workspacebuilder/three_pane.yaml")
...
def test_focus_pane_index(session):
    yaml_config = read_config_file("workspacebuilder/focus_and_pane.yaml")
...
def test_suppress_history(session):
    yaml_config = read_config_file("workspacebuilder/suppress_history.yaml")
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
    sconfig = config.trickle(sconfig)
...
def test_session_options(session):
    yaml_config = read_config_file("workspacebuilder/session_options.yaml")
    s = session
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_global_options(session):
    yaml_config = read_config_file("workspacebuilder/global_options.yaml")
    s = session
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_global_session_env_options(session, monkeypatch):
    ...
    yaml_config = read_config_file("workspacebuilder/env_var_options.yaml")
    s = session
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_window_options(session):
    yaml_config = read_config_file("workspacebuilder/window_options.yaml")
    s = session
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_window_options_after(session):
    yaml_config = read_config_file("workspacebuilder/window_options_after.yaml")
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_window_shell(session):
    yaml_config = read_config_file("workspacebuilder/window_shell.yaml")
    s = session
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_environment_variables(session):
    yaml_config = read_config_file("workspacebuilder/environment_vars.yaml")
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
    sconfig = config.expand(sconfig)
...
def test_automatic_rename_option(session):
    ...
    yaml_config = read_config_file("workspacebuilder/window_automatic_rename.yaml")
    s = session
    sconfig = kaptan.Kaptan(handler="yaml")
    sconfig = sconfig.import_config(yaml_config).get()
...
def test_start_directory(session, tmp_path: pathlib.Path):
    yaml_config = read_config_file("workspacebuilder/start_directory.yaml")
...
def test_start_directory_relative(session, tmp_path: pathlib.Path):
    ...
    """
    yaml_config = read_config_file("workspacebuilder/start_directory_relative.yaml")
...
def test_pane_order(session):
    ...
    yaml_config = read_config_file("workspacebuilder/pane_ordering.yaml").format(
...
def test_window_index(session):
    yaml_config = read_config_file("workspacebuilder/window_index.yaml")
    proc = session.cmd("show-option", "-gv", "base-index")
    base_index = int(proc.stdout[0])
...
def test_before_load_throw_error_if_retcode_error(server):
    config_script_fails = read_config_file("workspacebuilder/config_script_fails.yaml")
    sconfig = kaptan.Kaptan(handler="yaml")
...
def test_before_load_throw_error_if_file_not_exists(server):
    config_script_not_exists = read_config_file(
...
def test_before_load_true_if_test_passes(server):
    config_script_completes = read_config_file(
...
def test_plugin_system_before_workspace_builder(
    ...
    config_plugins = read_config_file("workspacebuilder/plugin_bwb.yaml")
...
def test_plugin_system_on_window_create(monkeypatch_plugin_test_packages, session):
    config_plugins = read_config_file("workspacebuilder/plugin_owc.yaml")
...
def test_plugin_system_after_window_finished(monkeypatch_plugin_test_packages, session):
    config_plugins = read_config_file("workspacebuilder/plugin_awf.yaml")
...
def test_plugin_system_on_window_create_multiple_windows(session):
    config_plugins = read_config_file(
...
def test_plugin_system_multiple_plugins(monkeypatch_plugin_test_packages, session):
    config_plugins = read_config_file("workspacebuilder/plugin_multiple_plugins.yaml")
...
def test_load_configs_same_session(server):
    ...
    yaml_config = read_config_file("workspacebuilder/two_windows.yaml")
