...
These exercise the live build path and require libtmux's unreleased chain API
(libtmux#685). The whole module is skipped on released libtmux, where the API is
absent. The import-guarded fallback (selecting ``chain`` without the API) is
covered by :mod:`tests.workspace.test_builder_chain_unavailable`.
"""
...
pytest.importorskip("libtmux._experimental.chain")
