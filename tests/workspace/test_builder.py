...
# --- respawn-pane provisioning tests ---
...
def test_here_mode_respawn_provisioning(
    ...
    """--here mode provisions the reused pane via respawn-pane."""
...
def test_here_mode_respawn_warns_on_running_processes(
    ...
    # Give the shell time to fork the background job; best-effort —
    # the pgrep warning check below is the real assertion
    retry_until(
        lambda: (
            "sleep" in (pane.pane_current_command or "") or len(pane.capture_pane()) > 1
        ),
        seconds=3,
        raises=False,
    )
