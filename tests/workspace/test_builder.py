...
def test_pane_readiness_waits_for_default_shell_panes(
    ...
    waited: list[str] = []
    original = builder_classic._wait_for_panes_ready
    ...
    monkeypatch.setattr(builder_classic, "_wait_for_panes_ready", recording_barrier)
...
def _count_readiness_waits(
    *,
    ...
    """Build a workspace from ``yaml`` and count panes sent to the readiness barrier.

    The concurrent builder waits for every default-shell pane once, in a single
    barrier (:func:`_wait_for_panes_ready`), rather than per pane. The
    pane_readiness policy resolved in ``build()`` decides whether the barrier
    runs at all, so counting the panes submitted to it reflects the policy.
    ...
    monkeypatch.setattr(builder_classic, "_wait_for_panes_ready", counting_barrier)
...
def test_layout_runs_after_readiness_barrier(
    ...
    """
    events: list[str] = []
    original_barrier = builder_classic._wait_for_panes_ready
    original_select_layout = Window.select_layout
    ...
    monkeypatch.setattr(builder_classic, "_wait_for_panes_ready", traced_barrier)
