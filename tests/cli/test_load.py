...
class WorkspaceConfigPathFixture(t.NamedTuple):
    """Test fixture for workspace tmux config path expansion."""

    test_id: str
    name: str
    workspace_config: str
    cli_tmux_config_file: str | None
    expect_config_file: str


WORKSPACE_CONFIG_PATH_FIXTURES: list[WorkspaceConfigPathFixture] = [
    WorkspaceConfigPathFixture(
        test_id="relative-config-resolves-from-workspace-file",
        name="relative",
        workspace_config="./tmux.conf",
        cli_tmux_config_file=None,
        expect_config_file="{workspace_dir}/tmux.conf",
    ),
    WorkspaceConfigPathFixture(
        test_id="home-config-expands",
        name="home-path",
        workspace_config="~/tmux.conf",
        cli_tmux_config_file=None,
        expect_config_file="{home_dir}/tmux.conf",
    ),
    WorkspaceConfigPathFixture(
        test_id="cli-config-file-takes-precedence",
        name="cli",
        workspace_config="./ignored.conf",
        cli_tmux_config_file="{fixture_tmux_conf}",
        expect_config_file="{fixture_tmux_conf}",
    ),
]


@pytest.mark.parametrize(
    list(WorkspaceConfigPathFixture._fields),
    WORKSPACE_CONFIG_PATH_FIXTURES,
    ids=[f.test_id for f in WORKSPACE_CONFIG_PATH_FIXTURES],
)
def test_load_workspace_expands_workspace_config_path(
    tmp_path: pathlib.Path,
    server: Server,
    monkeypatch: pytest.MonkeyPatch,
    test_id: str,
    ...
    """Workspace config fallback paths expand before Server creation."""
    monkeypatch.delenv("TMUX", raising=False)
    ...
    workspace_dir = tmp_path / "workspaces" / name
    workspace_dir.mkdir(parents=True)
    for config_file in (workspace_dir / "tmux.conf", home_dir / "tmux.conf"):
        config_file.write_text("", encoding="utf-8")

    fixture_tmux_conf = str(FIXTURE_PATH / "tmux" / "tmux.conf")
    ...
    def _resolve(val: str | None) -> str | None:
        ...
        return val.format(
            ...
            home_dir=home_dir,
            fixture_tmux_conf=fixture_tmux_conf,
        )

    workspace_file = workspace_dir / "test.yaml"
    workspace_file.write_text(
        f"""\
session_name: cfg-path-{test_id}
config: {_resolve(workspace_config)}
windows:
- window_name: main
  panes:
  - echo hello
